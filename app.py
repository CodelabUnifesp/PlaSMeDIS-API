...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URI',
                                                       "postgresql://jkpaprazxcpojo:2a135108dda110cdf26d9ef31fff1c6b9f94cd92993f25a90c3df353c685626d@ec2-52-45-179-101.compute-1.amazonaws.com:5432/d5bi00ifg35edj")
...
class Form_Socioeconomico(db.Model):
    ...
    def __init__(self, nome_rep_familia, pessoa, qtd_pessoas_familia, qtd_criancas, gestante, qtd_amamentando,
                 qtd_criancas_deficiencia, qtd_gestantes, pessoa_amamenta):
...
def form_socio(id):
    if request.method == 'POST':
        if request.is_json:
            data = request.get_json()
            new_form = Form_Socioeconomico(nome_rep_familia=data['nome_rep_familia'], pessoa=data['pessoa'],
                                           qtd_pessoas_familia=data['qtd_pessoas_familia'],
                                           pessoa_amamenta=data['pessoa_amamenta'], qtd_criancas=data['qtd_criancas'],
                                           gestante=data['gestante'], qtd_amamentando=data['qtd_amamentando'],
...
def form_socio_get_by_user(user_id):
    if request.method == 'POST':
        if request.is_json:
            ...
            if form is None:
                new_form = Form_Socioeconomico(nome_rep_familia=data['nome_rep_familia'], pessoa=user_id,
                                               qtd_pessoas_familia=data['qtd_pessoas_familia'],
                                               pessoa_amamenta=data['pessoa_amamenta'],
                                               qtd_criancas=data['qtd_criancas'], gestante=data['gestante'],
    ...
    if request.method == 'POST':
        if request.is_json:
            ...
            hashed_password = bcrypt.generate_password_hash(data['password']).decode(
                'utf-8')  # bcrypt here (create binary and turn into string by decoding it)
            new_user = Usuario(real_name=data['real_name'], password=hashed_password, user_name=data['user_name'],
            ...
            new_user_not = Notificacoes_Conf(usuario=new_user.id, sistema=False, selo_postagem=False,
                                             comentario_postagem=False, saude=False, lazer=False, trocas=False)
...
def login():
    ...
    if request.method == 'POST':
        if request.is_json:
            ...
            if user:  # could be one and would cancel X and X
                if bcrypt.check_password_hash(user.password,
                                              data['password']):  # should compare with hash with string given #AND?
                    expiration = datetime.datetime.utcnow() + datetime.timedelta(days=7)
                    issuedAt = datetime.datetime.utcnow()
                    token = jwt.encode({'auth': AUTH_VERSION, 'exp': expiration, 'iat': issuedAt, 'sub': user.id,
                                        'iss': os.environ.get('ME', 'plasmedis-api-local'),
                                        'aud': request.args.get('aud', 'unknown')}, app.config['SECRET_KEY'],
                                       algorithm="HS256")
                    return {"status": 1000, "user": toDict(user), "token": token,
                            "verificado": str(user.verificado)}  # Valido
                else:
                    return {"status": 1010}  # Invalido #COULD BE UNIFIED?
            else:
                return {"status": 1010}  # Invalido #COULD BE UNIFIED?
        else:
            return {"error": "A requisição não foi feita no formato esperado"}
...
def handle_user(id):  # Mod User
    ...
    if request.method == 'GET':
        ...
        # user.password = hashed_password #was data['password']
...
def postagens():
    if request.method == 'POST':
        ...
        postagens = Postagem.query.outerjoin(Comentario).add_columns(
            func.count(Comentario.id).label('comentarios')).group_by(Postagem.id)
...
def recomendados():
    if request.method == 'GET':
        postagens = db.session.query(Postagem, func.count(Comentario.id).label('comentarios')).outerjoin(
            Comentario).filter(Postagem.selo == True).group_by(Postagem.id)
...
def filtros(id_categoria):
    postagens = db.session.query(Postagem, func.count(Comentario.id).label('comentarios')).outerjoin(Comentario).filter(
        Postagem.categoria == id_categoria).group_by(Postagem.id)
...
def comentarios():
    if request.method == 'POST':
        if request.is_json:
            data = request.get_json()
            new_comment = Comentario(texto=data['texto'], criador=data['criador'], resposta=data['resposta'],
...
def esqueci_senha():
    if request.method == 'POST':
        if request.is_json:
            ...
            smtpObj.login('codelabtesteesquecisenha@gmail.com',
                          '44D6DDAAC9C660F72D6490D7CC44731BEA7C236A9241B387D3E9AF0C66B30D49')
